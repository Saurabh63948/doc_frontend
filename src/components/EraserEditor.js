...
/**
 * EraserEditor — Upgraded Version
 *
 * Canvas  → Konva.js  (via react-konva)
 *   npm install konva react-konva
 *
 * Rich text → custom execCommand editor, same as before but with
 *   link / code / highlight / align / indent added.
 *   (If you want TipTap, replace the <DocEditor> section)
 *   npm install @tiptap/react @tiptap/starter-kit @tiptap/extension-highlight
 *       @tiptap/extension-text-align @tiptap/extension-link @tiptap/extension-underline
 *
 * Everything else is React + Tailwind, no extra deps.
 */
...
import {
  ...
  Save, CheckCircle2, Loader2, Link, Highlighter,
  ZoomIn, ZoomOut, Maximize2, RotateCcw,
  Diamond, Minus,
} from "lucide-react";
import { useAuth } from "@/context/AuthContext";

// ─── Konva lazy-import (SSR safe) ────────────────────────────────────────────
// react-konva only renders in the browser.
// We lazy-load so Next.js doesn't break on server.
import dynamic from "next/dynamic";
const KonvaCanvas = dynamic(() => import("./_KonvaCanvas"), { ssr: false });
...
// ─── Rich-text toolbar ────────────────────────────────────────────────────────
function RichToolbar({ editorRef, onSave }) {
  const [fontSize, setFontSize] = useState("3");
  const [activeColor, setActiveColor] = useState("#e0e0e0");
  ...
  const groups = [
    ...
    [
      ...
      { icon: Minus,       label: "Divider",       cmd: "insertHorizontalRule" },
    ],
    [
      { icon: Code,        label: "Inline Code",   action: () => {
  ...
  return (<>
    <div className="flex items-center gap-2 px-4 py-2 bg-[#161616] border-b border-white/5 flex-wrap flex-shrink-0">
      {groups.map((group, gi) => (
        <React.Fragment key={gi}>
          <div className="flex items-center gap-0.5">
            {group.map((t, i) => {
              const Icon = t.icon;
              const handleAction = t.action || (() => exec(t.cmd));
              return (
                <button
                  key={i}
                  onMouseDown={(e) => { e.preventDefault(); handleAction(); }}
                  title={t.label}
                  className="px-2 py-1.5 rounded text-gray-400 hover:text-white hover:bg-white/8 transition-all text-[11px] font-bold"
                >
      ...
      <select
        ...
        className="bg-[#222] border border-white/10 text-gray-300 text-[11px] rounded px-2 py-1 outline-none cursor-pointer"
      >
        {[["1","10"],["2","12"],["3","14"],["4","16"],["5","18"],["6","24"],["7","32"]].map(([v,l]) => (
          <option key={v} value={v}>{l}px</option>
        ))}
      </select>

      {/* Text color */}
      <label className="relative cursor-pointer" title="Text Color">
        <div className="w-7 h-7 rounded border border-white/10 flex items-center justify-center" style={{ background: activeColor }}>
          <span className="text-[9px] font-bold" style={{ color: activeColor === "#000000" ? "#fff" : "#000" }}>A</span>
        </div>
        <input
          type="color"
          defaultValue="#e0e0e0"
          ...
          onMouseDown={(e) => e.stopPropagation()}
          onChange={(e) => { setActiveColor(e.target.value); exec("foreColor", e.target.value); }}
        />
      </label>
    </div></>
  );
...
function DocEditor({ file, docBodyRef, scheduleSave }) {
  return (
    <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10">
      <div className="max-w-2xl mx-auto px-10 py-10">
        ...
        <div
          ...
          spellCheck={false}
          onInput={scheduleSave}
          className="text-[2.2rem] font-black text-white mb-2 outline-none leading-tight
            empty:before:content-[attr(data-placeholder)] empty:before:text-white/15"
          ref={(el) => {
            if (el && !el.dataset.initialized) {
              el.dataset.initialized = "1";
              el.textContent = file?.name || "";
        ...
        />
        {/* Subtitle / meta line */}
        <div className="text-xs text-white/20 mb-8 pb-6 border-b border-white/5">
...
export default function EraserEditor({ file, onBack }) {
  ...
  useEffect(() => {
    if (shapes.length > 0) scheduleSave();
    ...
    >
      ...
      <nav className="h-12 border-b border-white/[0.06] flex items-center justify-between px-4 bg-[#141414] z-50 flex-shrink-0">
        ...
        <div className="flex items-center gap-3 min-w-0">
          <button
            onClick={onBack}
            className="hover:bg-white/8 p-1.5 rounded-lg transition-colors text-gray-400 hover:text-white"
          >
            <ChevronLeft size={18} />
          </button>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-100 truncate max-w-[200px]">
              {file?.name || "Untitled"}
            </span>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={saveToServer}
            className="text-[11px] font-semibold border border-white/10 px-3 py-1.5 rounded-lg
              hover:bg-white/5 flex items-center gap-2 transition-colors text-gray-400 hover:text-white"
          >
            <Save size={13} /> Save
          </button>
          <button className="text-[11px] font-semibold border border-white/10 px-3 py-1.5 rounded-lg
            hover:bg-white/5 flex items-center gap-2 transition-colors text-gray-400 hover:text-white">
            <Share2 size={13} /> Share
          </button>
          <button className="text-[11px] font-semibold bg-blue-600 px-3 py-1.5 rounded-lg
            hover:bg-blue-500 flex items-center gap-2 text-white transition-colors shadow-lg shadow-blue-900/30">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showDoc && (
          <section
            className={`flex flex-col h-full ${
              activeTab === "both"
                ? "w-1/2 border-r border-white/[0.05]"
                : "w-full"
            }`}
            style={{ background: "#111" }}
          >
            <RichToolbar editorRef={docBodyRef} onSave={saveToServer} />
            <DocEditor
              file={file}
        ...
        {showCanvas && (
          ...
          >
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
              <button
                className="pointer-events-auto flex items-center gap-3 px-5 py-2.5 rounded-2xl text-sm font-semibold
                  hover:border-blue-500 transition-all shadow-2xl border"
                style={{ background: "#161616", borderColor: "#2a2a2a" }}
              >
                <Sparkles size={15} className="text-blue-400" />
                <span className="text-gray-200">Generate AI Diagram</span>
                <kbd className="text-[10px] bg-[#222] px-1.5 py-0.5 rounded text-gray-500 border border-white/10">
                  ⌘ J
                </kbd>
              </button>
            </div>
      ...
      <style>{`
        ...
        .doc-body h1 {
          ...
          margin: 1.5rem 0 0.5rem; line-height: 1.2;
          font-family: 'system-ui', sans-serif;
          border-bottom: 1px solid #1f1f1f; padding-bottom: 0.4rem;
        }
        .doc-body h2 {
          font-size: 1.4rem; font-weight: 700; color: #e5e7eb;
        ...
        .doc-body ul { list-style: disc; padding-left: 1.6rem; margin: 0.6rem 0; }
        .doc-body ol { list-style: decimal; padding-left: 1.6rem; margin: 0.6rem 0; }
        .doc-body li { margin-bottom: 0.3rem; }
        .doc-body hr { border: none; border-top: 1px solid #1f1f1f; margin: 1.4rem 0; }
        .doc-body p  { margin-bottom: 0.6rem; }
        .doc-body a  { color: #60a5fa; text-decoration: underline; }
        .doc-body a:hover { color: #93c5fd; }
...
// ─── Canvas sidebar ───────────────────────────────────────────────────────────
function CanvasSidebar({ activeTool, setActiveTool, onClear }) {
  const tools = [
  ...
  return (
    <div
      className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col gap-1
        p-2 rounded-2xl shadow-2xl z-20 border border-white/[0.06]"
